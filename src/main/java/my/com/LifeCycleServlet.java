 ...
 * service() : 스레드에 의해 호출이 되는메서드,
 * 			- 요청방식이 get방식이면 이안에서 doget()호출
 * 			- 요청방식이 post             dopost()
 * 			- 		   delete           doDelete()
 * 			- 		   put              doput()
 * destroy() :서버가 shutdown(정상종료)될때 딱한번 호출되는 메서드
 * 			  비정상종료는 X
 */
...
public class LifeCycleServlet extends HttpServlet {
    ...
    public LifeCycleServlet() {
        super();
        System.out.println("라이프 사이클 생성자....");
    ...
    public void service(HttpServletRequest req,HttpServletResponse res)throws ServletException, IOException{
    	...
    	out.println("<h2>스레드이름 :"+tr.getName()+"</h2>");
    	Runtime rt=Runtime.getRuntime();
    	long memory=rt.freeMemory();
    	long memory3=rt.maxMemory();
    	
    	out.println("<h3>현재 가용 메모리"+memory+"</h3>");
    	out.println("<h3>최대 메모리"+memory3+"</h3>");
    	if(count>10) {
    		System.gc();
    		long memory2=rt.freeMemory();
    		out.println("<hr color='blue'>");
    		out.println("<h3>GC호출후 가용메모리:"+memory2+"</h3>");
    		count=0;
    	}
